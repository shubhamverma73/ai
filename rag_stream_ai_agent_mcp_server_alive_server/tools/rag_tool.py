...
# RAG Tool
#
# One short answer + sources at the end.
#
...
# Rteturn response + sources at once, not streaming.
#
# Agent requires this format
#   {
#       "answer": "...",
#       "sources": [...]
#   }
...
'''
Browsers and agents are different things.

    The browser is the user interface.

    The agent is the decision-maker.

Agnet need answer with sources at the end, so it can decide what to do next.
The browser just needs the answer, and sources can come later.
Because:
    In future, agent might decide to call another tool based on sources.:
        ...
        so required structured data.

But browser just needs the answer, so it can show sources at the end, so streaming is better for user experience and here we requre
typing...
typing...
typing...
Thats why we have two separate functions.
    rag_tool() for agent with structured output, to decide what to do next.
    stream_answer() for browser with streaming output
'''