...
Keep the live Flask application answer-only. Do not call RAGAS evaluation inside the request stream in [tools/rag_tool.py](tools/rag_tool.py).

Use a separate evaluation runner:

```bash
cd /d/xampp/htdocs/node/open_ai/rag_stream_ai_agent_test_ragas
./venv/Scripts/python.exe evaluation/ragas_runner.py
```

Update your dataset file at:

```text
evaluation/test_dataset.json
```

The dataset should contain sample questions and `ground_truth` values, for example:
...
The evaluation wrapper lives in:

```text
evaluation/ragas_eval.py
```

and the project RAG tool context retrieval is loaded from:
